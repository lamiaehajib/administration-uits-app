    ...
    <tbody>
        @foreach ($devisf as $devisItem)
            <tr>
                ...
                <td>
                    <a href="{{ route('devisf.show', $devisItem->id) }}" class="btn btn-info btn-sm" title="Voir" style="display:inline-block;">
                        <i class="fas fa-eye"></i> Voir
                    </a>

                    <form method="POST" action="{{ route('devisf.restore', $devisItem->id) }}" style="display:inline-block;">
                        @csrf
                        @method('PUT')
                    ...
                    <form method="POST" action="{{ route('devisf.forceDelete', $devisItem->id) }}" style="display:inline-block;">
                        @csrf
                        @method('DELETE')
