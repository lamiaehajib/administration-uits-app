<x-app-layout>
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('#category-filter').on('change', function() {
                ...
                $.ajax({
                    ...
                    success: function(response) {
                        if (response.success && response.produits.length > 0) {
                            ...
                            response.produits.forEach(p => {
                                options += `<option value="${p.id}" data-prix="${p.prix_vente}" data-stock="${p.quantite_stock}" data-has-variants="${p.has_variants}">
                    ...
                    error: function() { $('#filter-info').html('<span class="text-danger">❌ Erreur de chargement</span>'); }
                });
            });
            ...
            $(document).on('change', '.produit-select', function() {
                ...
                // Reset remise quand on change de produit
                resetRemiseRow(row);
                ...
                if (hasVariants) {
                    $.ajax({
                        ...
                        method: 'GET',
                        beforeSend: function() { row.find('.variant-info').text('🔄 Chargement...'); },
                        success: function(response) {
            ...
            // ===== RECALCUL SUR QUANTITÉ / TVA =====
            $(document).on('input', '.quantite-input, #tva', function() {
                // Si une remise est appliquée et la quantité change, recalculer remise montant
        ...
        function confirmerRemise() {
            ...
            if (type === 'montant' && valeur > currentRemiseSousTotal) {
                Swal.fire('Erreur', 'Remise ne peut pas dépasser le sous-total', 'error');
                return;
            }

            // Appliquer la remise sur la row
        ...
        function calculateTotal() {
            ...
            $('.item-row').each(function() {
                ...
                if (remiseAppliquee) {
                    if (remisePourcentage > 0) {
                        montantRemise = (ligneSousTotal * remisePourcentage) / 100;
                    } else {
            ...
            // Auto-fill montant payé si vide ou était = total précédent
