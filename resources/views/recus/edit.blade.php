<x-app-layout>
                ...
                <div class="card-body">
                    <div id="items-container">
                        ...
                        <div class="item-row gift-row mb-3 p-3 border border-success rounded bg-success bg-opacity-10"
                             data-index="{{ $index }}" data-is-gift="1">
                            <div class="d-flex align-items-center mb-2">
                                ...
                                <span class="ms-auto text-muted small">
                                    Coût: {{ number_format($item->prix_achat * $item->quantite, 2) }} DH → charge variable automatique
                                </span>
                            </div>
                            <div class="row g-2">
                                <input type="hidden" name="items[{{ $index }}][produit_id]"        value="{{ $item->produit_id }}">
                                <input type="hidden" name="items[{{ $index }}][is_gift]"            value="1">
                                <input type="hidden" name="items[{{ $index }}][prix_unitaire]"      value="0">
                                <input type="hidden" name="items[{{ $index }}][prix_achat]"         value="{{ $item->prix_achat }}">
                                <input type="hidden" name="items[{{ $index }}][achat_id]"           value="{{ $item->achat_id }}">
                                <input type="hidden" name="items[{{ $index }}][remise_appliquee]"   value="0">
                                <input type="hidden" name="items[{{ $index }}][remise_pourcentage]" value="0">
                                <input type="hidden" name="items[{{ $index }}][remise_montant]"     value="0">
                                <input type="hidden" class="is-gift-input"  value="1">
                                <input type="hidden" class="lot-prix-achat" value="{{ $item->prix_achat }}">
                                <input type="hidden" class="lot-prix-vente" value="0">

                                <div class="col-md-4">
                                    <label class="form-label small text-success fw-bold">Accessoire offert</label>
                                    <input type="text" class="form-control form-control-sm bg-white"
                                           value="{{ $item->produit_nom }}" readonly>
                                </div>
                                <div class="col-md-2">
                                    ...
                                    <input type="number" name="items[{{ $index }}][quantite]"
                                           ...
                                           value="{{ $item->quantite }}" min="1" required>
                                </div>
                                <div class="col-md-2">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text" class="form-control form-control-sm bg-light gift-cout-display"
                                           value="{{ number_format($item->prix_achat * $item->quantite, 2) }} DH" readonly>
                                </div>
                                <div class="col-md-1">
                            ...
                            <small class="text-muted mt-1 d-block">
                                ...
                                La charge gift sera recréée automatiquement lors de la mise à jour.
                            </small>
                        </div>
                            ...
                            <div class="row g-3">
                                    ...
                                    <div class="table-responsive">
                                        <table class="table table-sm table-bordered table-hover lots-table">
                                            <thead class="table-light">
                                                <tr>
                                                    <th>Choisir</th><th>Date Achat</th><th>Fournisseur</th>
                                                    <th>Stock Dispo</th><th>Prix Achat</th><th>Prix Vente</th><th>Marge</th>
                                                </tr>
                                            </thead>
                                            <tbody class="lots-tbody"></tbody>
                                        </table>
                                    </div>
                                    <input type="hidden" name="items[{{ $index }}][achat_id]"      class="lot-achat-id"   value="{{ $item->achat_id }}">
                                    <input type="hidden" name="items[{{ $index }}][prix_achat]"    class="lot-prix-achat" value="{{ $item->prix_achat }}">
                                    <input type="hidden" name="items[{{ $index }}][prix_unitaire]" class="lot-prix-vente" value="{{ $item->prix_unitaire }}">
                                </div>

                                <!-- Variant -->
                                <div class="col-md-3 variant-container"
                                     style="display: {{ $item->product_variant_id ? 'block' : 'none' }};">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <div class="input-group input-group-sm">
                                        <input type="hidden" name="items[{{ $index }}][remise_appliquee]"
                                               class="remise-appliquee-input"
                                               value="{{ $item->remise_appliquee ? '1' : '0' }}">
                                        <input type="hidden" name="items[{{ $index }}][remise_pourcentage]"
                                               class="remise-pourcentage-input"
                                               value="{{ $item->remise_pourcentage ?? 0 }}">
                                        <input type="hidden" name="items[{{ $index }}][remise_montant]"
                                               class="remise-montant-input"
                                               value="{{ $item->attributes['remise_montant'] ?? 0 }}">
                                        <input type="text" class="form-control remise-display bg-light" readonly
                                               ...
                                               placeholder="Aucune">
                                        <button type="button"
                                                class="btn {{ $item->remise_appliquee ? 'btn-info' : 'btn-warning' }} btn-remise">
                                ...
                                <div class="col-md-2">
                                    ...
                                    <input type="text"
                                           class="form-control total-ligne-display bg-light fw-bold {{ $item->remise_appliquee ? 'text-success' : '' }}"
                                           readonly
                                ...
                                <!-- Specs variant -->
                                <div class="col-12 variant-specs"
                                     style="display: {{ $item->variant ? 'block' : 'none' }};">
                                ...
                                <!-- Badge remise -->
                                <div class="col-12 remise-badge-container"
                                     style="display: {{ $item->remise_appliquee ? 'block' : 'none' }};">
                                    <span class="badge bg-warning text-dark">
                                        <i class="fas fa-tag me-1"></i>
                                        <span class="remise-badge-text">
                                            @if($item->remise_appliquee)
                                                @if($item->remise_pourcentage > 0)
                                                    Remise: {{ number_format($item->remise_pourcentage,2) }}%
                                                    (-{{ number_format(($item->sous_total * $item->remise_pourcentage)/100,2) }} DH)
                                                @else
                                        ...
                                        <button type="button"
                                                class="btn-close btn-close-sm ms-2 btn-supprimer-remise"
                ...
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            ...
                            <input type="text" id="sous-total-display" class="form-control bg-light"
                                   readonly value="{{ number_format($recu->sous_total, 2) }}">
                        </div>
                        <div class="col-md-3">
                            <label class="form-label">Total Remises (DH)</label>
                            @php $totalRemisesActuel = $recu->items->sum('montant_remise'); @endphp
                            <input type="text" id="remises-total-display"
                                   class="form-control bg-light text-danger" readonly
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header bg-warning">
                    <h5 class="modal-title"><i class="fas fa-tag me-2"></i>Appliquer Remise</h5>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-success text-white">
                    <h5 class="modal-title"><i class="fas fa-gift me-2"></i>Choisir un Accessoire à Offrir</h5>
                ...
                <div class="modal-body">
                    ...
                    <div id="gift-produits-list" style="display:none;">
                        <div class="alert alert-info py-2 mb-3">
                            ...
                            Sélectionnez l'accessoire à offrir gratuitement.
                        ...
                        <div class="row g-3" id="gift-produits-grid"></div>
                    </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-success" id="btn-confirm-gift" disabled>
                        <i class="fas fa-check me-2"></i>Confirmer le Gift
    ...
    // ================================================================
    // TEMPLATE OPTIONS PRODUITS (même logique que create)
    // ================================================================
    ...
    // ================================================================
    // GIFT SYSTEM — identique à create.blade.php
    // ================================================================
            ...
            grid.append(`
                <div class="col-md-4 col-sm-6 gift-produit-card-wrapper" data-nom="${p.nom.toLowerCase()}">
                    <div class="card h-100 border-2 gift-produit-card ${disabled}"
                         ...
                         data-prix-achat="${p.prix_achat}" data-prix-vente="${p.prix_vente}"
                        ...
                        <div class="card-body p-3">
                            ...
                            <small class="text-muted">
                                <i class="fas fa-tag me-1"></i>Coût: <strong>${parseFloat(p.prix_achat).toFixed(2)} DH</strong>
                            </small>
                        </div>
                        <div class="card-footer bg-light p-2 gift-qty-container" style="display:none;">
                            <label class="form-label small mb-1 fw-bold text-success">Quantité</label>
                            <div class="input-group input-group-sm">
    ...
    $(document).on('click', '.gift-produit-card', function () {
        ...
        selectedGiftProduit = {
            id: $(this).data('id'), nom: $(this).data('nom'),
    ...
    function updateGiftConfirmButton() {
        if (!selectedGiftProduit) { $('#btn-confirm-gift').prop('disabled', true); $('#gift-confirm-info').text(''); return; }
    ...
    function getSelectedGiftQty() {
        ...
        return activeCard.length ? (parseInt(activeCard.find('.gift-qty-input').val()) || 1) : 1;
    }
    ...
    $('#btn-confirm-gift').on('click', function () {
        ...
        Swal.fire({
            icon: 'success', title: '🎁 Gift ajouté !',
            html: `<strong>${selectedGiftProduit.nom}</strong> x${qty}<br>Coût: <strong>${cout} DH</strong> comme charge variable.`,
    ...
    function buildGiftRow(idx, produit, qty, firstLot) {
        ...
        return `
            <div class="item-row gift-row mb-3 p-3 border border-success rounded bg-success bg-opacity-10"
                ...
                <div class="d-flex align-items-center mb-2">
                    <span class="badge bg-success fs-6 me-2"><i class="fas fa-gift me-1"></i> GIFT</span>
                    <strong>${produit.nom}</strong>
                    <span class="ms-auto text-muted small">Coût: ${cout} DH → charge variable automatique</span>
                </div>
                <div class="row g-2">
                    ...
                    <div class="col-md-4">
                        <label class="form-label small text-success fw-bold">Accessoire offert</label>
                        <input type="text" class="form-control form-control-sm bg-white" value="${produit.nom}" readonly>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label small text-success fw-bold">Quantité</label>
    ...
    // ================================================================
    // TEMPLATE ITEM ROW — identique à create.blade.php
    // ================================================================
                ...
                <div class="row g-3">
                    ...
                    <div class="col-md-2">
                        ...
                        <div class="input-group input-group-sm">
                            ...
                            <button type="button" class="btn btn-warning btn-remise"><i class="fas fa-tag"></i></button>
                        </div>
                    </div>
                    <div class="col-md-2">
                        <label class="form-label">Total ligne</label>
    ...
    $(document).ready(function () {
        ...
        $('#reset-filter').on('click', function () { $('#category-filter').val('').trigger('change'); });
        ...
        $(document).on('change', '.produit-select', function () {
            ...
            const lots        = selectedOpt.data('lots') || [];
            const hasVariants = selectedOpt.data('has-variants') === true || selectedOpt.data('has-variants') === 'true';
                ...
                lots.forEach((lot, idx) => {
                    ...
                    tbodyHtml += `
                        <tr class="lot-row ${idx === 0 ? 'table-primary' : ''}" style="cursor:pointer;"
                            data-achat-id="${lot.id}" data-prix-achat="${lot.prix_achat}"
                            data-prix-vente="${lot.prix_vente_suggere}" data-stock="${lot.quantite_restante}"
                            data-marge="${lot.marge_pourcentage}">
                            <td class="text-center">
                                <input type="radio" class="form-check-input lot-radio" ${idx === 0 ? 'checked' : ''}>
                            </td>
        ...
        $(document).on('click', '.lot-row', function () {
            ...
            selectLot(row, $(this).data('achat-id'), $(this).data('prix-achat'),
                          $(this).data('prix-vente'), $(this).data('stock'));
            calculateTotal();
        });
        ...
        $(document).on('change', '.variant-select', function () {
            ...
            if (variantId) {
                row.find('.prix-display').val(parseFloat($(this).find(':selected').data('prix')).toFixed(2) + ' DH');
                row.find('.quantite-input').attr('max', $(this).find(':selected').data('stock'));
                ...
                if (lotPrix > 0) row.find('.prix-display').val(lotPrix.toFixed(2) + ' DH');
                row.find('.variant-specs').hide();
            }
            calculateTotal();
        });
        ...
        $(document).on('click', '.btn-remise', function () {
            ...
            if (pourcentage > 0) { $('#modal-type-remise').val('pourcentage'); $('#modal-valeur-remise').val(pourcentage); }
            else if (montant > 0) { $('#modal-type-remise').val('montant'); $('#modal-valeur-remise').val(montant); }
            else { $('#modal-type-remise').val('montant'); $('#modal-valeur-remise').val(''); }
        ...
        $(document).on('input', '.quantite-input, #tva', function () {
            if ($(this).hasClass('quantite-input')) {
                ...
                if (remiseAppliquee && remisePourcentage > 0) {
                    ...
                    const sousTotal = prix * (parseInt($(this).val()) || 0);
        ...
        $('#recuForm').on('submit', function (e) {
            ...
            $('.item-row').each(function () {
                ...
                const produitId = isGift
                    ? row.find('input[name*="produit_id"]').val()
                ...
                if (!produitId) { valid = false; errors.push('Sélectionnez un produit pour chaque ligne'); }
    ...
    // ================================================================
    // HELPERS — identiques à create.blade.php
    // ================================================================
    ...
    function toggleRemiseType() {
        ...
        if (type === 'pourcentage') {
            unite.textContent = '%'; input.max = 100; hint.textContent = 'Maximum: 100%';
        } else {
            unite.textContent = 'DH'; input.max = currentRemiseSousTotal;
    ...
    function calculateTotal() {
        let sousTotal = 0, totalRemises = 0;

        $('.item-row').each(function () {
            const row      = $(this);
            const isGift   = row.find('.is-gift-input').val() === '1' || row.data('is-gift') == '1';
    ...
    function updateRemoveButtons() {
        ...
        // Désactiver supprimer si 1 seule ligne normale reste
        $('.item-row:not(.gift-row) .remove-item').prop('disabled', normalRows <= 1);
        // Gift toujours supprimable
        $('.gift-row .remove-item').prop('disabled', false);
    }
    </script>
    @endpush
</x-app-layout>