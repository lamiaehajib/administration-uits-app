    ...
    <div class="container mt-5">
        <div class="card shadow-lg p-4">
            <h2 class="fw-bold text-primary mb-4">
                <i class="fas fa-edit"></i> Modifier une Vente
            </h2>
            ...
            <form action="{{ route('ventes.update', $vente->id) }}" method="POST" class="needs-validation" novalidate>
                @csrf
                @method('PUT')
                    ...
                    <select name="category_id" id="category_id" class="form-select" disabled>
                        ...
                        @foreach ($categories as $category)
                            <option value="{{ $category->id }}" 
                                @if ($vente->produit->category_id == $category->id) selected @endif>
                                {{ $category->nom }}
                            </option>
                        @endforeach
                    </select>
                        ...
                        @foreach ($categories as $category)
                            @foreach ($category->produits as $produit)
                                <option value="{{ $produit->id }}" 
                                    @if ($vente->produit_id == $produit->id) selected @endif>
                                    {{ $produit->nom }} (Stock: {{ $produit->quantite_stock }}, Prix: {{ $produit->prix_vendu }} DH)
                                </option>
                            @endforeach
                        @endforeach
                    </select>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="quantite_vendue" id="quantite_vendue" class="form-control" value="{{ $vente->quantite_vendue }}" min="1" required>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <input type="number" name="prix_vendu" id="prix_vendu" class="form-control" value="{{ $vente->prix_vendu }}" step="0.01" min="0" required>
                </div>
